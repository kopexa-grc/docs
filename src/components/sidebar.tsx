	...
	return (
		<>
			<Presence present={open}>
				{/** biome-ignore lint/a11y/noStaticElementInteractions: fumadocs import */}
				{/** biome-ignore lint/a11y/useKeyWithClickEvents: fumadocs import */}
				<div
					data-state={state}
		...
		function renderSidebarList(
			...
			return items.map((item, i) => {
				if (item.type === "separator") {
					if (Separator) return <Separator key={i.toString()} item={item} />;
					return (
						<SidebarSeparator
							key={i.toString()}
				...
				if (item.type === "folder") {
					...
					if (Folder)
						return (
							<Folder key={i.toString()} item={item} level={level}>
								{children}
							</Folder>
						);
					return (
						<PageTreeFolder key={i.toString()} item={item}>
							{children}
						</PageTreeFolder>
					);
				}
